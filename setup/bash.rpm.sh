...
function config_url {
    ...
    echo "https://dl.redpanda.com/public/redpanda/config.rpm.txt?$params" | sed 's/ /%20/g'
}
...
function import_gpg_keys {
    local text="Importing 'redpanda/redpanda' repository GPG keys into rpm ..."
    echo_running "$text"

    local rc=0

    local key_urls=(
        "https://dl.redpanda.com/public/redpanda/gpg.988A7B0A4918BC85.key"
    )
...
function setup_repository {
    ...
    local repofile="$tmpdir/redpanda-redpanda.repo"
    check_fetch_config

    local text="Fetching 'redpanda/redpanda' repository configuration ..."
    echo_running "$text"
    retry 3 "fetch_config > $repofile"
    echo_okfail "$text" || die "Could not fetch repository config!"

    local text="Installing 'redpanda/redpanda' repository via $manager ..."
    echo_running "$text"
    ...
    if test "$manager" == "yum" -o "$manager" == "dnf"; then
        ...
        retry 3 "$yum_dnf_config_manager --add-repo $repofile" &>$tmp_log
        local rc=$?
        elif test "$manager" == "microdnf"; then
        mv "$repofile" "/etc/yum.repos.d/redpanda-redpanda.repo"
        local rc=$?
    else
        retry 3 "zypper ar -f $repo_file $repofile" &>$tmp_log
        local rc=$?
    fi
    ...
    if test "$manager" == "yum" -o "$manager" == "dnf"; then
        retry 3 "$manager -q makecache -y --disablerepo='*' --enablerepo='redpanda-redpanda*'"
        local rc=$?
        elif test "$manager" == "microdnf"; then
        ...
        retry 3 "$manager makecache -y --disablerepo='*' --enablerepo='redpanda-redpanda*'"
        local rc=$?
    else
        retry 3 "zypper --gpg-auto-import-keys --non-interactive refresh redpanda-redpanda redpanda-redpanda-source" &>$tmp_log
        local rc=$?
    fi
    echo_okfail_rc $rc "$text" || {
